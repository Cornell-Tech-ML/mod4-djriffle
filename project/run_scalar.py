...
class Linear(minitorch.Module):
    ...
    def forward(self, inputs):
        # TODO: Implement for Task 1.5.
        outputs = [bias_param.value for bias_param in self.bias]

        # Iterate over each input and its corresponding weights
        for input_idx, input_value in enumerate(inputs):
            for output_idx in range(len(outputs)):
                # Add the product of the input and its corresponding weight to the output
                weight_value = self.weights[input_idx][output_idx].value
                outputs[output_idx] += input_value * weight_value
